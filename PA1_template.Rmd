...
The mean number of steps per day is `r step.mean` and the median number of steps per day is `r step.median`.
...
The 5-five minute interval that contains the maximum steps averaged across all days is `r maxint$interval` with `r maxint$stepavg` average steps taken.
...
There are `r na.len` incomplete rows in the data set (i.e. the row contains NA). In order to impute missing values, the median value for the interval the NA measurement occurs on is used to replace the NA value.
...
After replacing missing values with the interval's median value, the mean number of steps per day is `r stepnew.mean` and the median number of steps per day is `r stepnew.median`. These differ from the original data set by `r meandiff` and `r mediandiff` respectively. This imputing strategy increased the mean and median by a non trivial amount suggesting an alternative strategy might be considered.
